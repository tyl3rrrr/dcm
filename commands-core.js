...
function buildHelpCommand(getAllCommands) {
  return {
    ...
    async execute(interaction) {
      ...
      const categories = {
        '📌 Allgemein': ['antimdm', 'web', 'uptime', 'status', 'changelog', 'links', 'botinfo', 'help'],
        '🛡️ Moderation': ['kick', 'ban', 'timeout', 'warn', 'clear'],
        '🎫 Tickets': ['ticket-panel'],
        '⚙️ Einstellungen & Admin': ['settings', 'reload', 'automod-setup'],
      };

      const embed = new EmbedBuilder().setTitle('📖 Befehlsübersicht').setColor(0x5865f2);
