// deploy-commands.js
// Liest alle Commands aus ./commands aus und registriert sie bei Discord.
...
for (const file of commandFiles) {
  ...
  if (!command || !command.data) {
    console.warn(`Warnung: ${file} hat kein "data" Feld - wird übersprungen.`);
    continue;
  }
...
(async () => {
  try {
    console.log(`Starte Registrierung von ${commands.length} Slash-Command(s): ${commands.map((c) => c.name).join(', ')}`);
